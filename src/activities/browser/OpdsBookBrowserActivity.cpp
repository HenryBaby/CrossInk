...
std::string buildDownloadFolder(const OpdsServer& server, const OpdsEntry& book) {
  const std::string base = normalizeOpdsDownloadFolder(server.downloadFolder);
  if (server.folderOrganization != OpdsFolderOrganization::AUTHOR || book.author.empty()) return base;
  const size_t sep = base == "/" ? 0 : 1;
  if (base.size() + sep >= OPDS_DOWNLOAD_FOLDER_MAX_BYTES) return base;
  const size_t budget = OPDS_DOWNLOAD_FOLDER_MAX_BYTES - base.size() - sep;
  std::string author = StringUtils::sanitizeFilename(book.author, budget);
  if (author.empty()) return base;
  return base == "/" ? "/" + author : base + "/" + author;
}

std::string buildDownloadPath(const std::string& folder, const OpdsServer& server, const OpdsEntry& book) {
  std::string name = StringUtils::sanitizeFilename(buildBookFilenameBase(book, server.filenameFormat));
  name += ".epub";
  return folder == "/" ? "/" + name : folder + "/" + name;
}

bool applyResponseFilename(const std::string& folder, const std::string& response, std::string& path) {
  if (response.empty()) return false;
  const std::string clean = StringUtils::sanitizeFilename(response);
  if (!FsHelpers::hasEpubExtension(clean)) return false;
  const std::string target = folder == "/" ? "/" + clean : folder + "/" + clean;
  if (target == path) return true;
  if (Storage.exists(target.c_str())) Storage.remove(target.c_str());
  if (!Storage.rename(path.c_str(), target.c_str())) return false;
  path = target;
  return true;
}
...
void OpdsBookBrowserActivity::downloadBook(const OpdsEntry& book) {
  ...
  if (baseFolder != "/" && !Storage.exists(baseFolder.c_str()) && !Storage.mkdir(baseFolder.c_str())) {
    LOG_ERR("OPDS", "Could not create download folder %s", baseFolder.c_str());
    state = BrowserState::ERROR;
  ...
  if (folder != "/" && !Storage.exists(folder.c_str()) && !Storage.mkdir(folder.c_str())) {
    LOG_ERR("OPDS", "Could not create download folder %s", folder.c_str());
    state = BrowserState::ERROR;
    errorMessage = tr(STR_DOWNLOAD_FAILED);
    requestUpdate();
    return;
  ...
  if (result == HttpDownloader::OK) {
    if (!responseFilename.empty()) applyResponseFilename(folder, responseFilename, filename);
